...
class ActorNetwork(nn.Module):
    def __init__(self, input_shape, output_shape, n_features, n_vehicles, **kwargs):
        super(ActorNetwork, self).__init__()

        n_input = input_shape[-1]
        n_output = output_shape[0]

        self.n_vehicles = n_vehicles / 2
        self._h1 = nn.Linear(n_input, n_features)
        self._h2 = nn.Linear(n_features, n_features)
        self._h3 = nn.Linear(n_features, n_output)
    ...
    def forward(self, state):
        ...
        a = (torch.tanh(self._h3(features2)) + 1) * self.n_vehicles
...
if __name__ == "__main__":
    ...
    # RUN
    # subtract 2 evaluate iterations
    n_epochs = 1 + env.spec.iterations - 5 - 2
